...
def diagnose():
    ...
    try:
        # 使用正确的端点
        resp = requests.get(f"{API_BASE}/annotations",
        ...
        if resp.status_code == 200:
            ...
            for i, ann in enumerate(annotations[:5]):  # 只看前5个
                ...
                print(
                    f"  类型: {ann.get('annotation_type', ann.get('type', 'N/A'))}")
            ...
            shape_count = sum(1 for a in annotations if a.get(
                'annotation_type') == 'shape')
            note_count = sum(1 for a in annotations if a.get(
